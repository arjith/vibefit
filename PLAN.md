...
| 0.2 | Pulse design system (`@vibefit/ui` — tokens, Radix primitives, 15+ components) | ⬜ | 0.1 | Storybook renders all states |
| 0.3 | PostgreSQL schema + Drizzle ORM (16 tables, pgvector, migrations, seed) | ⬜ | 0.1 | Migration runs, seed populates |
| 0.4 | Auth system (JWT + refresh, bcrypt, rate limiting, Zod validation) | ⬜ | 0.1 | Auth integration tests pass |
| 0.5 | API scaffolding (Express + Zod + error handler + logging + health check) | ⬜ | 0.3,0.4 | `GET /health` → 200 |
